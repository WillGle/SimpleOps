...
EOF
  ...
  # Loki
  LOKI_CONFIG="services/monitoring/loki/local-config.yml"
...
EOF
  ...
  echo "✅ Configuration files checked/generated."
}
...
if [ $? -ne 0 ]; then
  echo "❌ Error: Docker Compose failed to start. Check the output above." >&2
  exit 1
fi
...
# --- Output URLs ---
echo -e "\n🌐 Services ready at:"
echo "GitLab: http://gitlab.local:8080"
